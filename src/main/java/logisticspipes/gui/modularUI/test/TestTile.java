package logisticspipes.gui.modularUI.test;
...
import com.cleanroommc.modularui.test.TestGuiContainer;
...
/**
 * Various test and demos for synced widgets, slots and JEI interactions. Anything that doesn't fall into any of those
 * categories goes into {@link TestGuis}.
 */
public class TestTile extends TileEntity implements IGuiHolder<PosGuiData> {
    ...
    public ModularPanel buildUI(PosGuiData guiData, PanelSyncManager syncManager, UISettings settings) {
        ...
        GenericListSyncHandler<Integer> numberListSyncHandler = GenericListSyncHandler.<Integer>builder()
                .getter(() -> this.serverInts).setter(v -> this.serverInts = v).serializer(PacketBuffer::writeInt)
                .deserializer(PacketBuffer::readInt).immutableCopy().build();
        ...
        DynamicSyncHandler dynamicSyncHandler = new DynamicSyncHandler().widgetProvider((syncManager1, packet) -> {
            ...
            ItemStackHandler handler = stackHandlerMap
                    .computeIfAbsent(item, k -> new ItemStackHandler(handlerSizeMap.getInt(k)));
            String name = Item.itemRegistry.getNameForObject(item);
            Flow flow = Flow.row();
            for (int i = 0; i < handler.getSlots(); i++) {
                int finalI = i;
                flow.child(
                        new ItemSlot().syncHandler(
                                syncManager1.getOrCreateSyncHandler(
                                        name,
                                        i,
                                        ItemSlotSH.class,
        ...
        DynamicLinkedSyncHandler<GenericListSyncHandler<Integer>> dynamicLinkedSyncHandler = new DynamicLinkedSyncHandler<>(
                numberListSyncHandler).widgetProvider((syncManager1, value1) -> {
                    List<Integer> vals = value1.getValue();
                    return Flow.row().widthRel(1f).coverChildrenHeight()
        ...
        panel.size(176, 210).child(
                Flow.row().name("Tab row").coverChildren().topRel(0f, 4, 1f)
                        .child(
                                new PageButton(0, tabController).tab(GuiTextures.TAB_TOP, -1)
                                        .overlay(new ItemDrawable(Blocks.chest).asIcon()))
                        .child(
                                new PageButton(1, tabController).tab(GuiTextures.TAB_TOP, 0)
                                        .overlay(new ItemDrawable(Blocks.ender_chest).asIcon())))
                .child(
                        new Expandable().name("expandable").top(0).leftRelOffset(1f, 1)
                                .background(GuiTextures.MC_BACKGROUND).excludeAreaInRecipeViewer()
                                .stencilTransform((r, expanded) -> {
                                    r.width = Math.max(20, r.width - 5);
                                    r.height = Math.max(20, r.height - 5);
                                }).animationDuration(500).interpolation(Interpolation.BOUNCE_OUT)
                                .collapsedView(
                                        new ItemDrawable(Blocks.crafting_table).asIcon().asWidget().size(20).pos(0, 0))
                                .expandedView(
                                        new ParentWidget<>().name("crafting tab").coverChildren()
                                                .child(
                                                        new ItemDrawable(Blocks.crafting_table).asIcon().asWidget()
                                                                .size(20).pos(0, 0))
                                                .child(
                                                        IKey.str("Expandable & Crafting Demo").asWidget().scale(0.7f)
                                                                .pos(20, 7))
                                                .child(
                                                        SlotGroupWidget.builder().row("III  D").row("III  O")
                                                                .row("III   ")
                                                                .key(
                                                                        'I',
                                                                        i -> new ItemSlot()
                                                                ...
                                                                .key(
                                                                        ...
                                                                        new ItemDisplayWidget()
                                                                                ...
                                                                                .displayAmount(true))
                                                                .build().margin(5, 5, 20, 5).name("crafting"))))
                .child(
                        Flow.col().name("main_col").sizeRel(1f).padding(7).childPadding(5).child(
                                new PagedWidget<>().name("paged").widthRel(1f).expanded().controller(tabController)
                                        .addPage(
                                                new ParentWidget<>().name("page 1").sizeRel(1f).child(
                                                        Flow.row().crossAxisAlignment(Alignment.CrossAxis.START).child(
                                                                Flow.col().name("buttons_and_values_col").widthRel(0.5f)
                                                                        .mainAxisAlignment(
                                                                                Alignment.MainAxis.SPACE_BETWEEN)
                                                                        .child(
                                                                                IKey.str("Storage slots").asWidget()
                                                                                        .scale(0.7f).padding(1))
                                                                        .child(
                                                                                        ...
                                                                                        .onMousePressed(mouseButton -> {
                                                                                            ...
                                                                                            return true;
                                                                                                ...
                                                                                                IKey.str(
                                                                                                        ...
                                                                                                        .scale(0.75f)))
                                                                        .child(
                                                                                Flow.row().name("cycle_button_row")
                                                                                        ...
                                                                                        .child(
                                                                                                new ToggleButton()
                                                                                                        ...
                                                                                                        .tooltipBuilder(
                                                                                                                ...
                                                                                                                t -> {
                                                                                                                    t.addLine(
                                                                                                                            "Wow! This button sure isnt selected! Not one bit!");
                                                                                                                    if (this.oversizedStorage
                                                                                                                            .getStackInSlot(
                                                                                                                                    this.cycleState)
                                                                                                                            != null) {
                                                                                                                        t.addLine(
                                                                                                        ...
                                                                                                        .tooltipBuilder(
                                                                                                                true,
                                                                                                                t -> t.addLine(
                                                                                                                        "Wow! This button IS selected! How cool!"))
                                                                                                        .tooltipAutoUpdate(
                                                                                                                true)
                                                                                                        .overlay(
                                                                                                                GuiTextures.CYCLE_BUTTON_DEMO
                                                                                                                        .getSubArea(
                                                                                                                                0,
                                                                                                                                0,
                                                                                                                                1,
                                                                                                        ...
                                                                                                        .overlay(
                                                                                                                GuiTextures.CYCLE_BUTTON_DEMO
                                                                                                                        .getSubArea(
                                                                                                                                ...
                                                                                                                                1))))
                                                                        .child(
                                                                                Flow.row().name("progress_row")
                                                                                        .height(18)
                                                                                        .mainAxisAlignment(
                                                                                                Alignment.MainAxis.SPACE_AROUND)
                                                                                        .child(
                                                                                                new ProgressWidget()
                                                                                                        .syncHandler(
                                                                                                                "progress")
                                                                                                        .texture(
                                                                                                                GuiTextures.PROGRESS_ARROW,
                                                                                                                20))
                                                                                        .child(
                                                                                                new ProgressWidget()
                                                                                                        .syncHandler(
                                                                                                                "progress")
                                                                                                        .texture(
                                                                                                                GuiTextures.PROGRESS_CYCLE,
                                                                                                                20)
                                                                                                        .direction(
                                                                                                                ProgressWidget.Direction.CIRCULAR_CW))))
                                                                .child(
                                                                        Flow.col().name("slots_col").widthRel(0.5f)
                                                                                .heightRelOffset(1f, -6) // space for
                                                                                                         // player sort
                                                                                                         // buttons
                                                                                .mainAxisAlignment(
                                                                                        Alignment.MainAxis.SPACE_BETWEEN)
                                                                                .child(
                                                                                        IKey.str("Oversized slots")
                                                                                                .asWidget().scale(0.7f)
                                                                                                .padding(1))
                                                                                .child(
                                                                                        SlotGroupWidget.builder()
                                                                                                ...
                                                                                                .key(
                                                                                                        ...
                                                                                                        i -> new ItemSlot()
                                                                                                                .slot(
                                                                                                                        new ModularSlot(
                                                                                                                                this.oversizedStorage,
                                                                                                                                i).ignoreMaxStackSize(
                                                                                                                                        true)))
                                                                                                .build())
                                                                                .child(
                                                                                        IKey.str("Phantom slots")
                                                                                                .asWidget().scale(0.7f)
                                                                                                .padding(1))
                                                                                .child(
                                                                                        SlotGroupWidget.builder()
                                                                                        ...
                                                                                        SlotGroupWidget.builder()
                                                                                                ...
                                                                                                .key(
                                                                                                        ...
                                                                                                        i -> new FluidSlot()
                                                                                                                .syncHandler(
                                                                                                                        new FluidSlotSyncHandler(
                                                                                                                                this.phantomFluidStorage,
                                                                                                                                i).phantom(
                                                                                                                                        true)))
                                                                                                .build()))))
                                        .addPage(
                                                new ParentWidget<>().name("dynamic_sync_page").sizeRel(1f).child(
                                                        Flow.col().name("page 4 col, dynamic widgets").child(
                                                                IKey.str(
                                                                        "Dynamic synced widget demo. Items act as keys to a unique storage with different amount of slots.")
                                                                        .asWidget().scale(0.7f))
                                                                .child(
                                                                        new ItemSlot().slot(
                                                                                new ModularSlot(
                                                                                        this.storageInventory0,
                                                                                        0).changeListener(
                                                                                                ((newItem,
                                                                                                        onlyAmountChanged,
                                                                                                        client,
                                                                ...
                                                                .child(
                                                                        IKey.str("Dynamic linked sync handler demo.")
                                                                                .asWidget().scale(0.7f).marginTop(6))
                                                                .child(
                                                                        new DynamicSyncedWidget<>().widthRel(1f)
                                                                                .coverChildrenHeight().syncHandler(
    ...
    public ModularPanel openSecondWindow(PanelSyncManager syncManager, IPanelHandler syncHandler) {
        ModularPanel panel = new Dialog<>("second_window", null).setDisablePanelsBelow(false)
                .setCloseOnOutOfBoundsClick(false).setDraggable(true).size(100, 100);
        ...
        IPanelHandler panelSyncHandler = syncManager.syncedPanel(
                "other_panel_2",
                true,
        ...
        panel.child(new TextFieldWidget().size(60, 14).pos(10, 80).value(new StringSyncValue(() -> s, v -> s = v)));
        panel.child(ButtonWidget.panelCloseButton())
                .child(
                        new ButtonWidget<>().size(10).top(14).right(4).overlay(IKey.str("O"))
                                .addTooltipLine("Opens another sub panel").onMousePressed(mouseButton -> {
                                    panelSyncHandler.openPanel();
                                    return true;
                                }))
                .child(IKey.str("2nd Panel").asWidget().pos(5, 5))
                .child(
                        SlotGroupWidget.builder().row("II").row("II")
                                .key(
                                        'I',
                                        i -> new ItemSlot().slot(new ModularSlot(smallStorage, i).slotGroup(slotGroup)))
                                .build().center())
                .child(
                        new CycleButtonWidget().size(16).pos(5, 5 + 11).value(num).stateOverlay(0, IKey.str("1"))
                                .stateOverlay(1, IKey.str("2")).stateOverlay(2, IKey.str("3"))
    ...
    public ModularPanel openThirdWindow(PanelSyncManager syncManager, IPanelHandler syncHandler,
        ...
        ModularPanel panel = new Dialog<>("third_window", null).setDisablePanelsBelow(false)
                .setCloseOnOutOfBoundsClick(false).setDraggable(true).size(50, 50);
        panel.child(ButtonWidget.panelCloseButton())
                .child(IKey.str("3rd Panel: " + integer.get()).asWidget().pos(5, 17));
        return panel;
    }
